...
from modules.whoami_functions import connection_check, check_updates, local_ip, machine_os, clear, payloads_number
        ...
        while True:
            ...
            if command == "create":
                while True:
                    ...
                    if command == "show payloads":
                        ...
                        if check_payload(command) == True:
                            ...
                            while True:
                                command = input(f"\n{GREEN}Whoami{END}>{payload}: ").lower()
                                ...
                                if command == "set name":
                                    ...
                                    print(f"\n{GREEN}[+]{END} Change Your Payload Host (Ex: {local_ip()}).")
                                    ...
                                    print(f"\n{GREEN}[+]{END} Change Your Payload Port (Ex: 4444)")
                            ...
                            print(f"\n{RED}[!]{END} The Value Specified For Payload Is Not Valid Or The Payload Dosen't Exist.")
                                    ...
                                    print(f"\n{GREEN}[+]{END} show payloads   : Show Available Whoami Payloads\n{GREEN}[+]{END} set payload     : Set The Payload You Want To Use\n{GREEN}[+]{END} clear           : Clear The Screen\n{GREEN}[+]{END} back            : Move back from the current context")
                ...
                while True:
                    ...
                    if command == "set host":
                        print(f"\n{GREEN}[+]{END} Change Your Listening Host (Ex: {local_ip()}) Or Just Type 'l' To Listen On Your Local Network Or Type 'p' To Listen Outside Network.\n")
                        ...
                        print(f"\n{GREEN}[+]{END} Change Your Listening Port (Ex: 4444).")
                        ...
                        print(f"\n{GREEN}[+]{END} set host		: Change Your Listening Host (Ex: set host) Or Just Type 'l' To Listen On Your Local Network Or Type 'p' To Listen Outside Network\n{GREEN}[+]{END} set port		: Change Your Port (Ex: set port)\n{GREEN}[+]{END} show options	: Show Listening Options [Host,Port]\n{GREEN}[+]{END} listen  		: Start Listeneing\n{GREEN}[+]{END} clear		: Clear\n{GREEN}[+]{END} back		: Move back from the current context")
