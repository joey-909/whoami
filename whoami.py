            ...
            if command == "create":
                ...
                while True:
                    ...
                    if command == "help":
                        ...
                        listening_host = input(f"{YELLOW}[-]{END} Enter Listening Host: ").lower()
                        if listening_host == "l":
                            listening_host = local_ip()

                        elif listening_host == "p":
                            listening_host = "0.0.0.0"

                        print(f"\n{GREEN}[+]{END} Listening Host => {listening_host}")

                    elif command == "set port":
                        print(f"\n{GREEN}[+]{END} Change Your Listening Port (Ex: set port 4444).\n")
                        try:
                            listening_port = int(input(f"\n{YELLOW}[-]{END} Enter Port: "))
                            print(f"\n{GREEN}[+]{END} Listen Port => {listening_port}")

                        except:
                            print(f"\n{RED}[!]{END} Please Enter A Valid Listening Port.")

                    elif command == "show options":
                        print(f"\n{GREEN}[+]{END} Your Listening Host: {listening_host}\n{GREEN}[+]{END} Your Listen Port: {listening_port}")

                    elif command == 'run':
                        if listening_host == None:
                            print(f"\n{RED}[!]{END} Please Add Listening Host.")

                        elif listening_port == None:
                            print(f"\n{RED}[!]{END} Please Add Listening Port.")

                        else:
                            whoami_listen(listening_host, listening_port)
